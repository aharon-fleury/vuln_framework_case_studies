...
stage <- plot_df$stage %>% unique()
  ...
  vuln_plot <- ggplot(t_vuln, 
                      aes(x = stressor, y = vuln, colour = stage)) +
    theme_ohara(base_size = 12) +
    geom_hline(data = mean_tot_vuln, aes(yintercept = vuln, colour = stage)) +
    # geom_hline(data = mean_tot_vuln, aes(yintercept = vuln)) +
    geom_point(aes(color = stage), size = 1, alpha = .6, width = .2, height = .02) +
    ...
    labs(title = paste0('Vulnerability using gap-filled traits'))+
    facet_wrap(~stage)
  
  plotfile <- sprintf('figs/yes_stage_vuln_plot_from_traits_all.png')
