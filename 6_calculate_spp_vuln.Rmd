...
for(t in stage) {# t <- taxa[6]
  ...
  vuln_plot <- ggplot(t_vuln, 
                      aes(x = stressor, y = vuln, colour = gap)) +
    theme_ohara(base_size = 12) +
    geom_hline(data = mean_tot_vuln, aes(yintercept = vuln, colour = gap)) +
    # geom_hline(data = mean_tot_vuln, aes(yintercept = vuln)) +
    geom_point(aes(color = gap), size = 1, alpha = .6, width = .2, height = .02) +
  ...
  plotfile <- sprintf('figs/gap_stage_vuln_plot_from_traits_all.png')
