...
# if(!file.exists(sens_int_file)) {
  ...
  spp_sens_list <- parallel::mclapply(taxon_vec, mc.cores = 1, FUN = summarize_sens_by_taxon)
...
ggsave(here('figs/BOTH_spp_sens_scores.png'), height = 6, width = 6, dpi = 300)
...
write_csv(top_3_sens, here_anx('External Data/int/BOTH_sensitivity_top3_stressors.csv'))
```
...
ggsave(here('figs/BOTH_spp_adcap_gen_scores.png'), height = 4, width = 4, dpi = 300)
...
ggsave(here('figs/BOTH_spp_adcap_spec_scores.png'), height = 6, width = 6, dpi = 300)
...
write_csv(wt_top_3_adcap, here_anx('External Data/int/BOTH_spec_top3_stressors.csv'))
```
...
write_csv(cleared_exposure, here_anx('External Data/int/BOTH_BOTH_cleared_exposure.csv'))
```
...
write_csv(spp_scores_mean, here_anx('External Data/int/BOTH_spp_scores_mean.csv'))
...
write_csv(spp_vuln_rescale_taxa,  here('_output/BOTH_spp_vuln_from_traits_tx.csv'))
write_csv(str_lookup,             here('_output/BOTH_spp_vuln_from_traits_str.csv'))
write_csv(spp_vuln_rescale_score, here('_output/BOTH_spp_vuln_from_traits_score.csv'))

### to reassemble just vuln scores:
spp_vuln_scores <- fread(here('_output/BOTH_spp_vuln_from_traits_score.csv')) %>%
  left_join(fread(here('_output/BOTH_spp_vuln_from_traits_str.csv'))) %>%
  left_join(fread(here('_output/BOTH_spp_vuln_from_traits_tx.csv'))) %>%
...
write_csv(spp_vuln_rescale_taxa, 
          here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_tx.csv'))
write_csv(str_lookup, 
          here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_str.csv'))
write_csv(spp_vuln_resc_all_scores, 
          here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_all_scores.csv'))

### to reassemble all scores incl components:
spp_vuln_scores_all <- fread(here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_all_scores.csv')) %>%
  left_join(fread(here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_str.csv'))) %>%
  left_join(fread(here_anx('External Data/3_vuln_score_traits/BOTH_spp_vuln_from_traits_tx.csv'))) %>%
  select(-vuln_tx_id, -vuln_str_id)
...
for(t in taxa) { # t <- taxa[6]
  ...
  plotfile <- sprintf('figs/BOTH_vuln_plot_from_traits_%s.png', t)
