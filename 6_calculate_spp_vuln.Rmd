...
spp_traits_hab_fixed <- spp_traits %>%
  ...
  group_by(gap, stage, category, trait) %>%
...
``` {r}
sens_int_file <- here_anx('External Data/int/sensitivity_by_spp_traits.csv')
...
# if(!file.exists(sens_int_file)) {
  ...
  summarize_sens_by_gap_stage <- function(t) {
    ...
    # ' species in taxon ', t, '...')
    out_df <- int_df %>%
      group_by(gap, stage, stressor) %>%
      summarize(sens_score = sum(sens_score, na.rm = TRUE), .groups = 'drop')
    return(out_df)
  }
  stage_vec <- spp_sens_raw$stage %>% unique() %>% sort()
  
  spp_sens_list <- parallel::mclapply(stage_vec, mc.cores = 1, FUN = summarize_sens_by_gap_stage)
...
p <- ggplot(plot_df, aes(x = stressor, y = sens_norm)) +
  geom_point(aes(colour = gap), size = .25, alpha = .6, height = 0) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5, size = 6)) +
  ylim(c(0, 1)) +
  facet_wrap(~ stage)

ggsave(here('figs/gap_stage_sens_scores.png'), height = 6, width = 6, dpi = 300)

knitr::include_graphics(here('figs/gap_stage_sens_scores.png'))
```
...
write_csv(top_3_sens, here_anx('External Data/int/gap_stage_sensitivity_top3_stressors.csv'))
```
...
p <- ggplot(spp_adcap_gen, aes(x = stage, y = adcap_gen_score)) +
  geom_jitter(aes(colour = gap),size = .25, alpha = .6, width = .2, height = 0) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))

ggsave(here('figs/gap_stage_spp_adcap_gen_scores.png'), height = 4, width = 4, dpi = 300)

knitr::include_graphics(here('figs/gap_stage_spp_adcap_gen_scores.png'))
```
...
ggsave(here('figs/gap_stage_spp_adcap_spec_scores.png'), height = 6, width = 6, dpi = 300)

knitr::include_graphics(here('figs/gap_stage_spp_adcap_spec_scores.png'))
```
...
DT::datatable(wt_top_3_adcap)

write_csv(wt_top_3_adcap, here_anx('External Data/int/gap_stage_spec_top3_stressors.csv'))
```
...
spp_exposure_wildlife_strike <- spp_traits %>%
  ...
  mutate(btm_only = str_detect(zone, 'benthic|demersal') & 
         ...
         taxon_excl = stage %in% c('crustacea_arthropods', 'molluscs'),
...
spp_exposure <- spp_exposure_wildlife_strike %>%
  select(gap, stage, exposure_mod, stressor) %>%
  bind_rows(spp_exposure_raw)

non_exposures <- spp_exposure %>% 
  group_by(gap, stage, stressor) %>% 
  mutate(n_gps = n_distinct(stage)) %>%
  filter(exposure_mod == 0) %>% 
  summarize(n_gps_no_exp = n_distinct(stage),
            n_gps = first(n_gps),
...
null_exposures <- spp_traits %>%
  select(gap, stage) %>%
  anti_join(spp_exposure, by = c('gap', 'stage')) %>%
  distinct()

cleared_exposure <- spp_exposure_wildlife_strike %>%
  filter(override)

write_csv(cleared_exposure, here_anx('External Data/int/gap_stage_cleared_exposure.csv'))
```
...
spp_scores_mean <- spp_sens %>%
  left_join(spp_adcap_gen, by = c('gap', 'stage')) %>%
  left_join(spp_adcap_spec, by = c('gap', 'stage', 'stressor')) %>%
  drop_na()

write_csv(spp_scores_mean, here_anx('External Data/int/gap_stage_spp_scores_mean.csv'))
...
spp_vuln_all <- spp_scores_mean %>%
  calc_vuln() %>%
  select(gap, stage, stressor, 
         sens_score, adcap_gen_score, adcap_spec_score, vuln_raw) %>%
  left_join(spp_exposure, by = c('gap', 'stage', 'stressor')) %>%
  mutate(vuln_raw = vuln_raw * exposure_mod) %>%
  distinct()

### Creating a new category of all new stages that don't include the originals to evaluate against original later
spp_vuln_all <- spp_vuln_all %>% 
  bind_rows(filter(spp_vuln_all, stage != 'original')) %>% 
  group_by(gap, stage, stressor) %>%
...
spp_vuln_rescale_stage <- spp_vuln_rescale %>%
  select(gap, stage) %>%
  distinct() %>%
  arrange(gap, stage) %>%
...
spp_vuln_rescale_score <- spp_vuln_rescale %>%
  left_join(spp_vuln_rescale_stage, by = c('gap', 'stage')) %>%
...
write_csv(spp_vuln_rescale_stage,  here('_output/gap_stage_spp_vuln_from_traits_tx.csv'))
write_csv(str_lookup,             here('_output/gap_stage_spp_vuln_from_traits_str.csv'))
write_csv(spp_vuln_rescale_score, here('_output/gap_stage_spp_vuln_from_traits_score.csv'))
...
write_csv(spp_vuln_rescale_stage, 
          here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_stage.csv'))
write_csv(str_lookup, 
          here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_str.csv'))
write_csv(spp_vuln_resc_all_scores, 
          here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_all_scores.csv'))

### to reassemble all scores incl components:
spp_vuln_scores_all <- fread(here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_all_scores.csv')) %>%
  left_join(fread(here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_str.csv'))) %>%
  left_join(fread(here_anx('External Data/3_vuln_score_traits/gap_stage_spp_vuln_from_traits_stage.csv'))) %>%
...
strs_to_keep <- c(#'biomass_removal',
                  ...
                  #'sst_rise',
                  'uv')
plot_df <- spp_vuln_scores_all %>% 
  # filter(stressor %in% strs_to_keep) %>%
  filter(gap == 'yes') %>% 
  distinct() %>%
  mutate(across(where(is.numeric), ~round(., 4)))

stage <- plot_df$stage %>% unique()
                

  t_vuln <- plot_df %>% 
    mutate(across(stage, factor, levels = c('original', 'all', 'adult', 'juvenile', 'gyre', 'hatchling'))) %>% 
    filter(stage != 'all') %>% 
    mutate(stressor = recode(stressor, bycatch = "Bycatch", 
                             eutrophication_nutrient_pollution = "Nutrient pollution",
                             habitat_loss_degradation = "Habitat degradation",
                             light_pollution = "Light pollution",
                             marine_heat_waves = "Marine heat waves",
                             oa = "Ocean acidification",
                             plastic_pollution_microplastic = "Microplastic pollution",
                             slr = "Sea level rise",
                             wildlife_strike = "Wildlife Strike",
                             uv = "UV radiation"))  %>%   
  arrange(stage, desc(vuln)) %>% 
  mutate(stressor = factor(stressor, levels = unique(stressor)))


  
  mean_str_vuln <- t_vuln %>%
    group_by(stage, stressor) %>%
    summarize(vuln = mean(vuln), .groups = 'drop')
  mean_tot_vuln <- t_vuln %>%
    group_by(gap, stage) %>%
    summarize(vuln = mean(vuln))
  geom_tot_vuln <- t_vuln %>% 
    group_by(stage) %>% 
    summarize(geom = exp(mean(log(geom))))
...
vuln_plot <- ggplot(t_vuln,
         aes(x = stressor, y = vuln, group = stage)) +
  geom_point(size = 2, aes(shape = stage)) +
  theme_light() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = 1),
          panel.grid.minor = element_blank()) +
  scale_y_continuous(name = "Vulnerability") +
  scale_x_discrete(name = "Stressor", labels = c("Biomass removal", "Bycatch", "Sea surface temperature rise", "Macroplastic entanglement", "Nutrient pollution", "Habitat degradation", "Invasive species", "Light pollution", "Organic pollution", "Wildlife strike", "Inorganic pollution", "Sedimentation", "Storm disturbance", "Air temperature", "Marine heat waves", "Microplastic pollution", "Poisons and toxins", "Sea level rise", "Noise pollution", "Ocean acidification", "Oceanographic", "Salinity", "UV radiation")) +
  scale_shape_manual(name = 'Stage', labels = c('Butt et al.', 'Adult', 'Juvenile', 'Gyre', 'Hatchling'), values = c(15, 0, 1, 2, 6)) +
  scale_colour_manual(name = 'Stage', labels = c('Butt et al.', 'Adult', 'Juvenile', 'Gyre', 'Hatchling'), values=cbPalette) +
  geom_hline(data = mean_tot_vuln, aes(yintercept = vuln, color = factor(stage)), size = 0.5) 
  # scale_linetype_manual(name = 'Stage', labels = c('Butt et al', 'Adult', 'Juvenile', 'Gyre', 'Hatchling'), values )
  # geom_hline(data = geom_tot_vuln, aes(yintercept = geom, color = factor(stage)))
  
  plotfile <- sprintf('figs/pub_fig2.tiff')
...
#### Plots renamed to Butt et al. and only looking at gap with yes.
``` {r, results = 'asis'}
strs_to_keep <- c('biomass_removal',
                  'bycatch',
                  'eutrophication_nutrient_pollution',
...
for(t in stage) {# t <- taxa[6]
  t_vuln <- plot_df %>% 
    mutate(stage = ifelse(stage == 'original', 'Butt et al. 2022', stage)) %>%
    mutate(across(stage, factor, levels = c('Butt et al. 2022', 'all', 'adult', 'juvenile', 'gyre', 'hatchling'))) %>%
    filter(gap == 'yes')
  mean_str_vuln <- t_vuln %>%
    group_by(gap, stage, stressor) %>%
    summarize(vuln = mean(vuln), .groups = 'drop')
  ...
  vuln_plot <- ggplot(t_vuln, 
    ...
    # geom_hline(data = mean_tot_vuln, aes(yintercept = vuln)) +
    geom_point(size = 1, alpha = .6, width = .2, height = .02) +
    ...
    labs(title = paste0('Vulnerability of Chelonia mydas'))+
    facet_wrap(~stage)
  
  plotfile <- sprintf('figs/stage_vuln_plot_from_traits_all.png')
  ggsave(plot = vuln_plot, filename = plotfile, 
         width = 8, height = 8, dpi = 300)
  # cat(sprintf('![](%s)\n', plotfile))
  } 
...
# x
# 
# ggsave(here('figs/BOTH_vuln_by_stress_all_taxa.png'), height = 6, width = 6, dpi = 300)
```
...
#   
#   plotfile <- sprintf('figs/BOTH_vuln_plot_by_stressor_%s.png', s)
#   ggsave(plot = vuln_plot, filename = plotfile, 
#          width = 8, height = 8, dpi = 300)
#   # cat(sprintf('![](%s)\n', plotfile))
}
...
# x
# 
# ggsave(here('figs/BOTH_vuln_taxa_all_stressors.png'), height = 6, width = 6, dpi = 300)
```
