...
match_df <- fread(here_anx('External Data/1_downfill_traits', 
                           'spp_match_downfill_spp_traits.csv')) %>%
  full_join(fread(here_anx('External Data/1_downfill_traits', 
...
gapfill_traits_all_spp <- match_tx_fix %>%
  select(species = spp, category, trait, trait_value, trait_prob, stage, gap, phylum:genus, match) %>%
  mutate(gapfill = 'none') %>%
...
```{r}
spp_to_stage <- match_tx_fix %>% 
  select(stage, gap, species = spp) %>% 
  distinct()

gf_stage <- traits_all_gf_clean %>%
  select(stage, gap, species) %>%
  distinct() %>%
  left_join(spp_to_stage, by = c('gap', 'stage', 'species')) %>%
  arrange(gap, stage) %>%
...
gf_spp_traits <- traits_all_gf_clean %>%
  left_join(gf_stage, by = c('gap', 'stage')) %>%
...
write_csv(gf_stage,       
          here_anx('External Data/2_gapfill_traits/spp_up_down_gapfill_taxa.csv'))
write_csv(gf_traits,     
          here_anx('External Data/2_gapfill_traits/spp_up_down_gapfill_traits.csv'))
write_csv(gf_levels,     
          here_anx('External Data/2_gapfill_traits/spp_up_down_gapfill_levels.csv'))
write_csv(gf_spp_traits, 
          here_anx('External Data/2_gapfill_traits/spp_up_down_gapfill_spp_traits.csv'))
